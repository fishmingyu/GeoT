      ...
      if (avg <= 1.15) {
        if (feature_size <= 3.0) {
          ...
          if (size <= 1406130.0) {
            ...
            gather_scatter_pr_sorted<scalar_t, 2, 2, 4, 1, 16>(
                src_index, dst_index, src, dst);
          }
        }
      } else {
    ...
    if (feature_size <= 48.0) {
      if (feature_size <= 24.0) {
        if (size <= 20974.5) {
          if (avg <= 9.39) {
            if (size <= 7155.0) {
              gather_scatter_sr_sorted<scalar_t, 1, 32, 4, 8>(
                  src_index, dst_index, src, dst);
            } else {
              gather_scatter_sr_sorted<scalar_t, 1, 32, 4, 8>(
                  src_index, dst_index, src, dst);
            ...
            if (avg <= 35.43) {
              gather_scatter_sr_sorted<scalar_t, 1, 16, 4, 8>(
                  src_index, dst_index, src, dst);
            } else {
              gather_scatter_sr_sorted<scalar_t, 1, 16, 4, 8>(
                  src_index, dst_index, src, dst);
          ...
          if (feature_size <= 12.0) {
            if (size <= 93022.5) {
              gather_scatter_sr_sorted<scalar_t, 1, 8, 4, 8>(
                  src_index, dst_index, src, dst);
            } else {
              gather_scatter_sr_sorted<scalar_t, 1, 8, 8, 8>(
                  src_index, dst_index, src, dst);
            }
          } else {
            if (size <= 247962.5) {
              gather_scatter_sr_sorted<scalar_t, 1, 16, 8, 8>(
                  src_index, dst_index, src, dst);
            } else {
              gather_scatter_sr_sorted<scalar_t, 2, 8, 8, 8>(
                  src_index, dst_index, src, dst);
            }
          }
        }
        ...
        if (size <= 55061.5) {
          if (size <= 6521.0) {
            if (size <= 6413.5) {
              ...
              gather_scatter_sr_sorted<scalar_t, 1, 64, 8, 4>(
                  src_index, dst_index, src, dst);
            }
          } else {
            if (avg <= 502.41) {
          ...
          if (avg <= 4.22) {
            if (size <= 2351251.0) {
              gather_scatter_sr_sorted<scalar_t, 2, 16, 8, 8>(
                  src_index, dst_index, src, dst);
            } else {
              gather_scatter_sr_sorted<scalar_t, 2, 16, 32, 8>(
                  src_index, dst_index, src, dst);
            }
          } else {
            if (size <= 220246.0) {
      ...
      if (feature_size <= 96.0) {
        if (size <= 10487.0) {
          if (avg <= 1.15) {
            ...
            if (avg <= 38.97) {
              gather_scatter_sr_sorted<scalar_t, 1, 64, 4, 8>(
                  src_index, dst_index, src, dst);
            } else {
              gather_scatter_sr_sorted<scalar_t, 1, 64, 4, 8>(
                  src_index, dst_index, src, dst);
          ...
          if (size <= 1094118.0) {
            if (avg <= 492.02) {
              gather_scatter_sr_sorted<scalar_t, 2, 32, 8, 8>(
                  src_index, dst_index, src, dst);
            } else {
              gather_scatter_sr_sorted<scalar_t, 2, 32, 16, 8>(
                  src_index, dst_index, src, dst);
            }
          } else {
            if (size <= 1127504.0) {
              gather_scatter_sr_sorted<scalar_t, 2, 32, 32, 8>(
                  src_index, dst_index, src, dst);
            } else {
              gather_scatter_sr_sorted<scalar_t, 2, 32, 16, 8>(
                  src_index, dst_index, src, dst);
        ...
        if (size <= 18918.5) {
          if (size <= 5341.5) {
            if (size <= 3142.5) {
              ...
              gather_scatter_sr_sorted<scalar_t, 2, 64, 4, 4>(
                  src_index, dst_index, src, dst);
            }
          } else {
            if (avg <= 36.3) {
              gather_scatter_sr_sorted<scalar_t, 2, 32, 8, 4>(
                  src_index, dst_index, src, dst);
            } else {
              gather_scatter_sr_sorted<scalar_t, 2, 64, 8, 4>(
                  src_index, dst_index, src, dst);
            }
          }
        } else {
          if (size <= 747442.0) {
            if (avg <= 492.8) {
              ...
              gather_scatter_sr_sorted<scalar_t, 2, 64, 16, 4>(
                  src_index, dst_index, src, dst);
            }
          } else {
            if (avg <= 15.04) {
