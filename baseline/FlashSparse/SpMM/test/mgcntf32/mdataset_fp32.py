...
class MGCN_dataset(torch.nn.Module):
    ...
    def init_edges(self):
        ...
        self.row_pointers, self.column_index, self.degrees=FS_Block.blockProcess_tf32(self.row_pointers1, self.column_index1, self.dd, 8,4 )
        # self.row_pointers, self.column_index, self.degrees, self.t_window_rowTensor, self.t_atomicTensor=FS_Block.blockProcess_tf32_balance(self.row_pointers1, self.column_index1, self.dd, 8,4,2 )
    ...
    def init_embedding(self,dimN):
        ...
        self.x = torch.randint(low=1, high=3, size=(self.num_nodes_ori, dimN)).float()
